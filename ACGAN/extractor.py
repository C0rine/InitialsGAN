...
def extract(sep_letter, feature, csv, img_folder):
	...
	if len(sep_letter) > 1 or not sep_letter.isalpha():
		raise ValueError('Input can only be one letter')
	else:
		print('Will seperate letter \'' + sep_letter + '\'')
	sep_letter = sep_letter.upper()


	# check if image folder is already present
	if not os.path.exists(img_folder):
		# if not create one and make sure we don't skip the image saving
	    os.makedirs(img_folder)
	    skip_img = False
	else:
		skip_img = True
		print('Image seperation can be skipped')


	# Check if the csv is already present
	path_csv = Path(csv)
	if not path_csv.is_file():
		# then we need to create one
		skip_csv = False
		print('Will create csv for letter \'' + sep_letter + '\', and feature \'' + feature + '\'')
		# Lets already create the top row	
		if feature == 'countries':
			...
			raise ValueError('This is a not a feature to seperate on: ' + feature)
	...
	# if both the images and csv are already present we can return
	if skip_img and skip_csv:
		print('CSV is also already present')
		print('No new extraction needed')
		return
	# else we can continue with the rest of the code which will assume it at least needs to create the csv
	...
	for i in range(len(data)):
		...
		if letter == sep_letter:
			...
			if not skip_img:
				# Save the image
				utils.save_image(image, img_folder + image_name + '.jpg')
